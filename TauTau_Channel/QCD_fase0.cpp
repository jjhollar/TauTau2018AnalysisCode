...
//              cout<<"ola1";
    ...
    ifilelum.open("dadosluminosidade.txt");
//      cout<<"ola2";
    int lum[447][101] ;
    vector<int> lumaux;
        int d;
 ...
 out.Branch("generator_weight", &entry_30, "generator_weight/D");
	...
	check=0;
        ...
        for(int j=1; j<101 ;j=j+2)
        {
            ...
            if (luminosity>lum[lumcheck][j] && luminosity<lum[lumcheck][j+1])
            {
             	check=1;
                continue;
            }
		...
		if( tree->GetLeaf("HLT_DoubleMediumChargedIsoPFTauHPS35_Trk1_eta2p1_Reg")->GetValue(0)==1 ){

		if( tree->GetLeaf("Tau_pt")->GetLen()<=1) continue;
