...
int main(){
	...
	string prefix_output = "ttjets_2018_UL_skimmed_ETau_nano_";
	...
	,entry_23,entry_24,entry_25,entry_26,entry_27,entry_28, entry_30, entry_31, entry_32;
	int entry_29;
...
out.Branch("tau_decay", &entry_31, "tau_decay/D");
 out.Branch("tau_genmatch", &entry_32, "tau_genmatch/D");
		...
		if( tree->GetLeaf("HLT_IsoMu24")->GetValue(0)==1){    //garante a presença de um muão
			...
			if ( tree->GetLeaf("Muon_pt")->GetLen()>0 && tree->GetLeaf("Tau_pt")->GetLen()>0){ //garante a presençã de um muão e de um tau hadronico
				nmutau++;
										...
										entry_31=tree->GetLeaf("Tau_decayMode")->GetValue(0);
                    								entry_32=tree->GetLeaf("Tau_genPartFlav")->GetValue(0);
										out.Fill();
