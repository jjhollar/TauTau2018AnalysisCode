...
// samples for DY  region
          ...
          TFile dy("/eos/user/m/mpisano/samples_2018_emu/fase1/DY_fase1_PICskimmed_EMu_2018_total-proton.root");
          TFile ttjets("/eos/user/m/mpisano/samples_2018_emu/fase1/ttjets_fase1_PICskimmed_EMu_2018_total-proton.root");
          TFile qcd("/eos/user/m/mpisano/samples_2018_emu/fase1/DY_fase1_PICskimmed_EMu_2018_total-proton.root");

	TFile sinal("EMu_sinal_SM_july.root");


	TFile output("JHtest_e_mu_UL_2018_shapes.root","RECREATE","");
	...
	int n_evt_sinal = tree_sinal->GetEntries();
		...
		tau_qcd.Fill(tree_qcd->GetLeaf("e_pt")->GetValue(0),w_qcd);
	...
	m_qcd.SetLineWidth(0);
		...
		tau_dy.Fill(tree_dy->GetLeaf("e_pt")->GetValue(0),w_dy);
	...
	aco_dy.SetLineWidth(0);
		...
		tau_ttjets.Fill(tree_ttjets->GetLeaf("e_pt")->GetValue(0),w_ttjets);
	...
	for(int i=0; i<n_evt_sinal; i++){
                ...
                w_sinal = tree_sinal->GetLeaf("weight_sm")->GetValue(0)*1000;
		...
		tau_sinal.Fill(tree_sinal->GetLeaf("e_pt")->GetValue(0),w_sinal);
                met_sinal.Fill(tree_sinal->GetLeaf("muu_pt")->GetValue(0),w_sinal);
        }
	}
	...
	l_aco->AddEntry(&aco_sinal, "Signal (x1000)", "l");
        aco->Draw("histo");
	...
	l_m->AddEntry(&m_sinal, "Signal x 1000", "l");
        m->Draw("histo");
	...
	l_r->AddEntry(&r_sinal, "Signal (x 1000)", "l");
        r->Draw("histo");
	...
	l_pt->AddEntry(&pt_sinal, "Signal (x 1000)", "l");
        pt->Draw("histo");
	...
	TCanvas c5;
        ...
        l_mm->AddEntry(&mm_sinal, "Signal (x 1000)", "l");
        mm->Draw("histo");
	...
	TCanvas c6;
        ...
        l_ra->AddEntry(&ra_sinal, "Signal (x 1000)", "l");
        ra->Draw("histo");
	...
	TCanvas c7;
        ...
        l_tau->AddEntry(&tau_sinal, "Signal (x 1000)", "l");
        tau->Draw("histo");
        tau->GetXaxis()->SetTitle("Electron transverse momentum [GeV]");
	...
	TCanvas c8;
        ...
        l_met->AddEntry(&met_sinal, "Signal (x 1000)", "l");
        met->Draw("histo");
