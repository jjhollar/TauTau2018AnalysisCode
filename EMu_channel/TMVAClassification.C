...

{
   ...
   TString fnameBKG = "/eos/user/m/mpisano/samples_2018_emu/fase1/allBkgs_fase1_PICSkimmed_EMu_2018_total-proton_syst.root";

   TString fnameSIGN = "./EMu_sinal_SM_july.root";

   //TString fnameSIGN = "/eos/user/m/mpisano/Canal_Etau/skimmed_correct/ETau_2017_UL_sgn_BSM_up_skimmed.root";

   //TString fnameSIGN = "fake.root";
   ...
   if (!gSystem->AccessPathName(fnameBKG) || !gSystem->AccessPathName(fnameSIGN) ) {
      ...
      inputSIGN = TFile::Open( fnameSIGN ); // check if file in local directory exists
   ...
   std::cout << "--- TMVAClassification       : Using input BKG file: " << inputBKG->GetName() << std::endl;

   std::cout << "--- TMVAClassification       : Using input SIGNAL file: " << inputSIGN->GetName() << std::endl;
   ...
   TString outfileName( "Output_TMVA_tautau_2018.root");
  ...
  // dataloader->AddVariable("tau_pair_rapidity","Central system rapidity","", 'F');

   //dataloader->AddVariable("tau_pair_M","Central system invariant mass","GeV", 'F');
   ...
   //dataloader->AddVariable( "n_b_jet", "Number of bjets", "",'F');

   dataloader->AddVariable( "sist_mass", "Invariant Mass", "GeV",'F');
   
   dataloader->AddVariable( "sist_rap", "Sistem Rapidity", "",'F');

   dataloader->AddVariable( "met_pt", "Missing Energy", "GeV",'F');
   
  // dataloader->AddVariable( "tau_pair_Y-0.5*log(xi_1/xi_2)", "Y_X matching condition", "",'F');

  // dataloader->AddVariable("n_tau_h","Number of hadronic taus","",'F');
  ...
  // dataloader->AddVariable("min_deltaR_tau_h", "Minimum angular distance between a couple of non-leading tau_h", "", 'F');
   ...
   dataloader->AddVariable("mu_pt", "Muon Pt", "", 'D');

   dataloader->AddVariable("e_pt", "Electron Pt", "GeV" ,'D');

   dataloader->AddVariable("sist_rap-0.5*log(xi_arm1_1/xi_arm2_1)","Rapidity matching", "" ,'D');



   dataloader->SetSignalWeightExpression("weight_sm*weights_bsm_sf[51]");
...
//   dataloader->AddSpectator( "spec2 := var1*3",  "Spectator 2", "units", 'F' );
   ...
   //Double_t signalWeight     = signalTree->GetLeaf("weight")->GetValue(0);

   //Double_t backgroundWeight = background->GetLeaf("weight")->GetValue(0);




   //if(signalWeight>=0) signalWeight=signalWeight/0.012;
...
//  #################################################################################################################################
   ...
   dataloader->PrepareTrainingAndTestTree( mycuts, mycutb,
                                           ...
                                           "nTrain_Signal=2500:nTrain_Background=4300:SplitMode=Random:NormMode=NumEvents:!V" );

                                           // "nTrain_Signal=2000:nTrain_Background=32000:SplitMode=Random:NormMode=NumEvents:!V" );
...
//                                        "nTrain_Signal=1000:nTrain_Background=1000:SplitMode=Random:NormMode=NumEvents:!V" );
   ...
   if (Use["FDA_MC"])
      ...
      factory->BookMethod( dataloader, TMVA::Types::kFDA, "FDA_MC",
                           ...
                           "H:!V:Formula=(0)+(1)*x0+(2)*x1:ParRanges=(-1,1);(-10,10);(-10,10):FitMethod=MC:SampleSize=100000:Sigma=0.1" );
...
//                           "H:!V:Formula=(0)+(1)*x0+(2)*x1+(3)*x2+(4)*x3:ParRanges=(-1,1);(-10,10);(-10,10);(-10,10);(-10,10):FitMethod=MC:SampleSize=100000:Sigma=0.1" );
   ...
   if (Use["FDA_SA"]) // can also use Simulated Annealing (SA) algorithm (see Cuts_SA options])
      ...
      factory->BookMethod( dataloader, TMVA::Types::kFDA, "FDA_SA",
                           ...
                           "H:!V:Formula=(0)+(1)*x0:ParRanges=(-1,1);(-10,10);(-10,10):FitMethod=SA:MaxCalls=15000:KernelTemp=IncAdaptive:InitialTemp=1e+6:MinTemp=1e+6:MinTemp=1e-6:Eps=1e-10:UseDefaultScale" );
   ...
   if (Use["FDA_MT"])
      ...
      factory->BookMethod( dataloader, TMVA::Types::kFDA, "FDA_MT",
                           ...
                           "H:!V:Formula=(0)+(1)*x0+(2)*x1:ParRanges=(-1,1);(-10,10);(-10,10):FitMethod=MINUIT:ErrorLevel=1:PrintLevel=-1:FitStrategy=2:UseImprove:UseMinos:SetBatch" );
   ...
   if (Use["FDA_GAMT"])
      ...
      factory->BookMethod( dataloader, TMVA::Types::kFDA, "FDA_GAMT",
                           ...
                           "H:!V:Formula=(0)+(1)*x0+(2)*x1:ParRanges=(-1,1);(-10,10);(-10,10):FitMethod=GA:Converger=MINUIT:ErrorLevel=1:PrintLevel=-1:FitStrategy=0:!UseImprove:!UseMinos:SetBatch:Cycles=1:PopSize=5:Steps=5:Trim" );
   ...
   if (Use["FDA_MCMT"])
      ...
      factory->BookMethod( dataloader, TMVA::Types::kFDA, "FDA_MCMT",
                           ...
                           "H:!V:Formula=(0)+(1)*x0+(2)*x1:ParRanges=(-1,1);(-10,10);(-10,10):FitMethod=MC:Converger=MINUIT:ErrorLevel=1:PrintLevel=-1:FitStrategy=0:!UseImprove:!UseMinos:SetBatch:SampleSize=20" );
