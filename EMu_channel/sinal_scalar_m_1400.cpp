...
//ficheiro para as correções e sistemáticas
...
double EleReco(double pt, double eta, int flag=0){
 ...
 double entry_61, entry_62, entry_63, entry_64, entry_65, entry_66, entry_67, entry_68, entry_69, entry_71, entry_72, entry_73, entry_74, entry_75, entry_76, entry_77, entry_78, entry_79;
 vector<float> entry_70;
...
xi_sist_2->Fit("xi_sist_inter_2");
 ...
 TLorentzVector ele, mu, nu;

 TFile fundo ("/eos/user/m/mpisano/analyzer_emu/CMSSW_12_5_0/src/ExclTauTau/ExclTauTau/test/scalar_1400_gamma137/Data_EMuTriggers_2018_UL_sgn_BSM_scalar_1400_nov.root");
 TTree* ntp1 = (TTree*) fundo.Get("ntp1");
 TFile output ("./EMu_sinal_BSM_scalar_m1400.root", "RECREATE", "");
 ...
 TTree out("tree","");
 out.Branch("e_pt", &entry_1 ,"e_pt/D");
 out.Branch("mu_pt", &entry_2, "muu_pt/D");
 out.Branch("electron_charge", &entry_3, "electron_charge/D");
 out.Branch("mu_charge", &entry_4, "mu_charge/D");
 out.Branch("electron_eta", &entry_5, "electron_eta/D");
 out.Branch("mu_eta", &entry_6, "mu_eta/D");
 out.Branch("electron_phi", &entry_7, "electron_phi/D");
 out.Branch("mu_phi", &entry_8, "mu_phi/D");
 out.Branch("electron_id", &entry_9, "electron_id/D");
 ...
 out.Branch("mu_id", &entry_10, "mu_id/D");
 ...
 out.Branch("mu_energy", &entry_50, "mu_energy/D");
 ...
 out.Branch("n_eletron", &entry_63, "n_eletron/D");
 out.Branch("n_b_jet", &entry_64, "n_b_jet/D");
 out.Branch("weight_sm", &entry_65, "weight_sm/D");
 out.Branch("sist_rap", &entry_69, "sist_rap/D");
 //out.Branch("weights_bsm_sf",&entry_70);
...
for (int i = 0; i < ntp1 -> GetEntries(); i++){
	...
	ntp1 -> GetEvent(i);
	ntp1 -> SetBranchAddress("e_pt", &e_pt, &e_ptB);
	ntp1 -> SetBranchAddress("mu_pt", &mu_pt, &mu_ptB);
	ntp1 -> SetBranchAddress("e_charge", &e_charge, &e_chargeB);
	ntp1 -> SetBranchAddress("mu_charge", &mu_charge, &mu_chargeB);
	ntp1 -> SetBranchAddress("e_id", &e_id, &e_idB);
	ntp1 -> SetBranchAddress("mu_id", &mu_id, &mu_idB);
	ntp1 -> SetBranchAddress ("mu_eta", &mu_eta, &mu_etaB);
	ntp1 -> SetBranchAddress ("e_eta", &e_eta, &e_etaB);
	ntp1 -> SetBranchAddress ("mu_phi", &mu_phi, &mu_phiB);
	ntp1 -> SetBranchAddress ("e_phi", &e_phi, &e_phiB);	
	ntp1 -> SetBranchAddress ("mu_energy", &mu_energy, &mu_energyB);
	ntp1 -> SetBranchAddress ("e_energy", &e_energy, &e_energyB);
	...
	ntp1 -> SetBranchAddress ("proton_thx", &thx, &thxB);
        ...
        ntp1 -> SetBranchAddress ("mu_decay", &mu_decay, &mu_decayB);
	...
	//ntp1 -> SetBranchAddress ("bsm_weights", &bsm_weights, &bsm_weightsB);
        ...
        if ((e_charge)->size()==0 || (mu_charge)->size()==0) continue;

        weight_sample = 54900.*1/(4000.*993.);
           	        ...
           	        phi_e = (*e_phi)[0];
                                        ...
                                        if ((*mu_phi)[0] < 0.0) {
                                                phi_mu = (*mu_phi)[0] + 2*PI;
                                                }

                                        delta_phi =(abs((phi_e) - (phi_mu)));
	acop = (delta_phi /PI);
	...
	eta_e = (*e_eta)[0];
	eta_mu = (*mu_eta)[0];
	delta_eta = (abs((eta_e) - (eta_mu)));
	angular_dist = sqrt( ((delta_phi) * (delta_phi)) + ((delta_eta) * (delta_eta))); 

	if ((*mu_id)[0]>0.5 && (*e_id)[0] >0.5 && abs((*mu_eta)[0]) < 2.4 && abs((*e_eta)[0]) < 2.4 && angular_dist> 0.4){
        double mu_trig_sf = MuonTrigSF((*mu_pt)[0], (*mu_eta)[0], (*mu_charge)[0], (*mu_pt)[0]/sin(2.*atan(exp(-(*mu_eta)[0]))));
	double mu_idiso_sf = MuonIDISOSF((*mu_pt)[0], (*mu_eta)[0], (*mu_charge)[0], (*mu_pt)[0]/sin(2.*atan(exp(-(*mu_eta)[0]))));
	double mu_reco_sf = muRecoSF((*mu_pt)[0], (*mu_eta)[0]);
	double ele_trig_sf= EleTrig((*e_pt)[0], (*e_eta)[0]);
	double ele_idiso_sf= EleIDISO((*e_pt)[0], (*e_eta)[0]);
	double ele_reco_sf = EleReco((*e_pt)[0], (*e_eta)[0]);

	n_id= n_id + weight_sample*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	
	if ((*mu_pt)[0] >35. && (*e_pt)[0] >35.0 ){
        n_pt = n_pt + weight_sample*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;

	if (float((*e_charge)[0]) *float((*mu_charge)[0]) <0.0){
	n_charge = n_charge + weight_sample*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	


	if (abs((*mu_eta)[0]) < 2.4 && abs((*e_eta)[0]) < 2.4) {
	n_eta = n_eta + weight_sample*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;

	double mu_trig_syst = MuonTrigSF((*mu_pt)[0], (*mu_eta)[0], (*mu_charge)[0], (*mu_pt)[0]/sin(2.*atan(exp(-(*mu_eta)[0]))),1);
        double mu_idiso_syst = MuonIDISOSF((*mu_pt)[0], (*mu_eta)[0], (*mu_charge)[0], (*mu_pt)[0]/sin(2.*atan(exp(-(*mu_eta)[0]))),1);
        double ele_trig_syst= EleTrig((*e_pt)[0], (*e_eta)[0],1);
        double ele_idiso_syst= EleIDISO((*e_pt)[0], (*e_eta)[0],1);
	double mu_reco_syst = muRecoSF((*mu_pt)[0], (*mu_eta)[0],1);
	...
	mu.SetPtEtaPhiE((*mu_pt)[0],(*mu_eta)[0],(*mu_phi)[0],(*mu_energy)[0]);
	...
	n_rec=n_rec+weight_sample*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	...
	//FIDUCIAL CUTS
		   ...
		   cout << "#######################ehi" << endl;
		    n_p_up_xi=n_p_up_xi+weight_sample*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
		  ...
		  if(fraction >0.29 && fraction<=0.37) {
		   ...
		   //Era D2
		    ...
		    n_p_dw_xi=n_p_dw_xi+weight_sample*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	...
	n_p_rad=n_p_rad+weight*weight_sample*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	...
	entry_65=weight_sample*weight*mu_trig_sf*mu_idiso_sf*mu_reco_sf*ele_trig_sf*ele_idiso_sf*ele_reco_sf;
	//entry_70=(*bsm_weights);
...
cout << "os números apresentados têm em conta o sample weight" << endl;
